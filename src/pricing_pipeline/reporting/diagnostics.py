"""Side-effect-free aggregate diagnostics for already-scored predictions."""
...
class DiagnosticSections:
    """Named, aggregate-only sections consumed by report rendering."""

    metrics: pd.DataFrame
    importance: Mapping[str, pd.DataFrame]
    relativities: Mapping[str, Any]
    interactions: Mapping[str, Any]
    distributions: Mapping[str, Any]
    movement: Mapping[str, Any]
    curves: Mapping[str, Any]
    double_lift: Mapping[str, Any]
...
def calculate_diagnostics(
    ...
    """Calculate every aggregate report section without rendering or I/O."""
    exact_losses = {
    ...
    row_deviance = {
        name: _unit_tweedie_deviance(
            inputs.actual,
            prediction,
            options.resolved_tweedie_power,
        )
        for name, prediction in inputs.predictions.items()
    }
    metrics = _metrics_table(
        inputs,
        power=options.resolved_tweedie_power,
        exact_losses=exact_losses,
        row_deviance=row_deviance,
    )
    importance = _importance_payload(evidence)
    ...
    return DiagnosticSections(
        ...
        interactions=_interaction_payload(evidence),
        distributions=_prediction_distributions(
            inputs,
            n_bins=options.distribution_bins,
        ),
        movement=prediction_movement_payload(
            inputs.predictions,
            inputs.weight,
            inputs.comparison_unit_codes,
            n_bins=options.movement_bins,
            minimum_cell_size=options.minimum_cell_size,
        ),
        curves=_curve_payload(
            inputs,
            n_bins=options.curve_bins,
            minimum_cell_size=options.minimum_cell_size,
        ),
        double_lift=_double_lift_payload(
            inputs,
            n_bins=options.double_lift_bins,
            power=options.resolved_tweedie_power,
            exact_losses=exact_losses,
            row_deviance=row_deviance,
            bootstrap_replicates=options.comparison_bootstrap_replicates,
            bootstrap_seed=options.comparison_bootstrap_seed,
            minimum_cell_size=options.minimum_cell_size,
        ),
    )
