    ...
    if type(value) is dict or (allow_frozen and isinstance(value, Mapping)):
        ...
        for key, item in value.items():
            if not isinstance(key, str):
                raise ValueError(f"{path} metadata keys must be strings")  # noqa: TRY004
    ...
    *,
    ...
    for term_name, metadata in value.items():
        if not isinstance(term_name, str):
            raise ValueError("term_metadata metadata keys must be strings")  # noqa: TRY004
        ...
        for key, item in value.items():
            if not isinstance(key, str):
                raise ValueError(  # noqa: TRY004
                    "SuperGLM metadata mapping keys must be strings"
                )
...
def _grouping_metadata(grouping: LevelGrouping | None) -> dict[str, Any] | None:
    ...
    if not isinstance(grouping, LevelGrouping):
        raise ValueError(  # noqa: TRY004
            "SuperGLM categorical grouping must be a LevelGrouping"
        )
    return {
...
def _lambda_policy_metadata(
    ...
    if not isinstance(policy, Mapping):
        raise ValueError(  # noqa: TRY004
            "SuperGLM spline lambda_policy has an unsupported shape"
        )
    ...
    for component_name, component_policy in policy.items():
        if not isinstance(component_name, str) or not isinstance(component_policy, LambdaPolicy):
            raise ValueError(  # noqa: TRY004
                "SuperGLM spline lambda_policy must map names to LambdaPolicy"
            )
...
def _ordered_items(
    ...
    *,
    ...
    if not isinstance(specs, dict) or not isinstance(order, list):
        raise ValueError(  # noqa: TRY004
            f"SuperGLM {label} metadata has an invalid fitted shape"
        )
